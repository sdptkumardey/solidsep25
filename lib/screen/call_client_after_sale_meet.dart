// call_client_after_sale_meet.dart
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
class AfterSaleMeetScreen extends StatefulWidget {
  static const String id = 'call_client_after_sale_meet';

  const AfterSaleMeetScreen({super.key});

  @override
  State<AfterSaleMeetScreen> createState() => _AfterSaleMeetScreenState();
}

class _AfterSaleMeetScreenState extends State<AfterSaleMeetScreen> {
  bool _didInit = false;
  ...
  String? _satisfiedVal; // 'Yes' or 'No'
  ...
  Future<void> _saveStage() async {
    ...
    if (_satisfiedVal == null || _satisfiedVal!.isEmpty) {
      _showSnack('Please select satisfaction (Yes/No).');
      return;
    }
    ...
    try {
      ...
      // ✅ Use the correct single endpoint
      final url = Uri.parse(_endpoint(
        base, 'call_client_after_sale_meet.php?subject=call&action=stage',
      ));
      ...
      final resp = await http.post(url, body: {
        ...
        'satisfied': _satisfiedVal!, // Yes / No
      });
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        Scaffold(
          appBar: AppBar(title: const Text('After Sale Meet')),
          body: SafeArea(
            child: ListView(
              ...
              children: [
                ...
                // Satisfied dropdown (mandatory)
                const _FieldLabel('Satisfied with this product ? *'),
                const SizedBox(height: 6),
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (v) => setState(() => _satisfiedVal = v),
