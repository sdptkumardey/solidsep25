...
class NegotiationScreen extends StatefulWidget {
  static const String id = 'call_client_negotiation';
...
class _NegotiationScreenState extends State<NegotiationScreen> {
  ...
  final _numSheetCtrl = TextEditingController(); // mandatory
  final _offeredPriceCtrl = TextEditingController(); // optional
  final _wantedPriceCtrl  = TextEditingController(); // optional
  ...
  Future<void> _saveStage() async {
    ...
    // num sheet mandatory
    final sheetsStr = _numSheetCtrl.text.trim();
    if (sheetsStr.isEmpty) {
      _showSnack('No. of Sheet Taking is required.');
      return;
    }
    if (int.tryParse(sheetsStr) == null) {
      _showSnack('No. of Sheet must be a number.');
      return;
    }
    ...
    try {
      ...
      final url = Uri.parse(_endpoint(
          base, 'call_client_negotiation.php?subject=call&action=stage'));
      ...
      final body = {
        ...
        'num_sheet': sheetsStr,
        'offered_price': _offeredPriceCtrl.text.trim(),
        'wanted_price': _wantedPriceCtrl.text.trim(),
      };
          ...
          body: SafeArea(
            child: ListView(
              ...
              children: [
                ...
                // Offered / Wanted Price (numeric; optional)
                const _FieldLabel('Price Offered :'),
                const SizedBox(height: 6),
                TextField(
                ...
                const _FieldLabel('Price Wanted :'),
                const SizedBox(height: 6),
                TextField(
                  controller: _wantedPriceCtrl,
                  keyboardType: TextInputType.number,
                  inputFormatters: [FilteringTextInputFormatter.digitsOnly],
                  decoration: _decoration(),
                ),
